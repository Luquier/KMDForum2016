	...
	public class NGUIBarkUI : MonoBehaviour, IBarkUI {
		...
		public void Update() {
			if (secondsLeft > 0) {
				...
				if ((secondsLeft <= 0) && (label != null) && !(waitForContinueButton || waitUntilSequenceEnds)) Hide();
			}
		}
		...
		public void Bark(Subtitle subtitle) {
			if (label != null) {
                ...
                if (waitForContinueButton) {
                    var button = label.GetComponentInChildren<UIButton>();
                    if (button == null) {
                        waitForContinueButton = false;
                    } else {
                        var del = new EventDelegate();
                        del.target = this;
                        del.methodName = "OnContinue";
